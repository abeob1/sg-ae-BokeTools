...
#Region "Open Tools Category Screen"
    Private Sub OpenToolsCategory(ByVal objForm As SAPbouiCOM.Form, ByVal iLine As Integer)
        Dim sQuoteDocNo, sQuoteSeries, sCardCode, sItemCode, sToolCatNo, sBaseEntry As String
        ...
        sBaseEntry = oMatrix.Columns.Item("45").Cells.Item(iLine).Specific.value
...
#Region "Open Repair Notes Screen"
    Private Sub OpenRepairNotes(ByVal objForm As SAPbouiCOM.Form, ByVal iLine As Integer)
        Dim sQuoteDocNo, sQuoteSeries, sCardCode, sItemCode, sReprNtsDocNo, sToolCatNo, sBaseEntry As String
        Dim iCount As Integer
        ...
        If sBaseEntry = "" Then
            ...
            System.Runtime.InteropServices.Marshal.ReleaseComObject(oRecordSet)
        Else
            sReprNtsDocNo = oMatrix.Columns.Item("U_REPAIRNOTES").Cells.Item(iLine).Specific.value
            iCount = 1
        End If
...
#Region "Delete Sub Forms Data"
    Private Sub DeleteSubFormData(ByVal objForm As SAPbouiCOM.Form)
        ...
        For i As Integer = 1 To oMatrix.RowCount
            If oMatrix.Columns.Item("1").Cells.Item(i).Specific.value <> "" And oMatrix.Columns.Item("45").Cells.Item(i).Specific.value = "" Then
...
#Region "Delete Unchecked values in Tools Category Selection"
    Private Sub DelUncheckValues(ByVal objForm As SAPbouiCOM.Form)
        sSql = "DELETE FROM [@AE_TCS1]  WHERE U_SELECT = 'N' " & _
               " AND DocEntry = (SELECT DocEntry FROM [@AE_TCSS] WHERE U_BASEDOCNO = '" & sDocNo & "' AND U_BASEDOCSERIES = '" & sSeries & "' AND U_DOCTYPE = '13') "
        oRecordSet = p_oDICompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
        oRecordSet.DoQuery(sSql)
        System.Runtime.InteropServices.Marshal.ReleaseComObject(oRecordSet)
    End Sub
...
#Region "Item Event"
    Public Sub SalesInvoice_SBO_ItemEvent(ByVal FormUID As String, ByVal pval As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean, ByVal objForm As SAPbouiCOM.Form)
...
#End Region
#Region "Menu Event"
    Public Sub SalesInvoice_SBO_MenuEvent(ByVal pVal As SAPbouiCOM.MenuEvent, ByRef BubbleEvent As Boolean)
        Try
